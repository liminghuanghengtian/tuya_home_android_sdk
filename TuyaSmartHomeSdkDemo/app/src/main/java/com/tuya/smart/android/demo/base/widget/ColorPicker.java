...
public class ColorPicker extends View {
    ...
    private static final int[] HALO_COLORS = new int[]{0x80FFFFFF, 0x80FFFFFF};
    public static final int S_WITH_NO_RESPONING = 1;
    public static final int S_RESPONING = 0;
    /**
    ...
    private Paint mColorWheelHaloPaint;
    private Paint mCircleHaloPaint;
    private int mColorCircleHaloThickness;
    private boolean mShowColorWheel = false;
    private float mColorWheelRadiusScale = 1f;
    private float mCenterCircleLayerScaleX;
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        mColorCircleHaloThickness = a.getDimensionPixelSize(R.styleable.ColorPicker_color_circle_halo_thickness,
                b.getDimensionPixelSize(R.dimen.color_circle_halo_thickness));
        a.recycle();
...
//        Shader shader = new SweepGradient(0, 0, HALO_COLORS, null);
//        mColorWheelHaloPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
//        mColorWheelHaloPaint.setStyle(Paint.Style.STROKE);
//        mColorWheelHaloPaint.setStrokeWidth(mColorWheelThickness);
//        mColorWheelHaloPaint.setShader(shader);
//
        mCircleHaloPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mCircleHaloPaint.setStyle(Paint.Style.FILL);
        mCircleHaloPaint.setPathEffect(new CornerPathEffect(mColorCircleHaloThickness));
        mCircleHaloPaint.setColor(Color.parseColor("#EAEAEA"));
        mCircleHaloPaint.setStrokeWidth(mColorCircleHaloThickness);
        mCircleHaloPaint.setShadowLayer(mColorCircleHaloThickness, 0,
                0, Color.parseColor("#80FFFFFF"));
...
//			canvas.drawArc(mCenterRectangle, 0, 360, true, mCenterNewPaint);
        canvas.drawArc(mCenterRectangle, 0, 360, true, mCircleHaloPaint);
//        canvas.drawArc(mCenterRectangle, 0, 360, true, mPointerHaloPaint);
//        canvas.drawOval(mColorWheelRectangle, mColorWheelHaloPaint);
//		}
    }

    public void setCenterCircleColor(int color) {
        mCircleHaloPaint.setColor(color);
        invalidate();
    }

    public void setCenterCircleLayer(float scale) {
        mCenterCircleLayerScaleX = scale;
        float width = scale * mColorCircleHaloThickness;
        mCircleHaloPaint.setPathEffect(new CornerPathEffect(width));
        mCircleHaloPaint.setStrokeWidth(width);
        mCircleHaloPaint.setShadowLayer(width, 0,
                0, Color.argb((int) (255 * (0.4 + 0.4 * scale)), 255, 255, 255));
        invalidate();
    }

    public float getCenterCircleLayer() {
        return mCenterCircleLayerScaleX;
    }

    public int getCenterCircleColor() {
        return mCircleHaloPaint.getColor();
    }
     ...
     */
    public void setColor(int color, int status) {
        ...
        if (mOpacityBar != null) {
            ...
            mOpacityBar.setColor(mColor, status);
            mOpacityBar.setOpacity(Color.alpha(color), status);
        }
        ...
        if (mSVbar != null) {
            ...
            if (mHSV[1] < mHSV[2]) {
                mSVbar.setSaturation(mHSV[1], status);
            } else if (mHSV[1] > mHSV[2]) {
                mSVbar.setValue(mHSV[2], status);
            }
        }

        if (mSaturationBar != null) {
            Color.colorToHSV(color, mHSV);
            mSaturationBar.setColor(mColor, status);
            mSaturationBar.setSaturation(mHSV[1], status);
        }

        if (mValueBar != null && mSaturationBar == null) {
            Color.colorToHSV(color, mHSV);
            mValueBar.setColor(mColor, status);
            mValueBar.setValue(mHSV[2], status);
        } else if (mValueBar != null) {
            Color.colorToHSV(color, mHSV);
            mValueBar.setValue(mHSV[2], status);
        }
        setNewCenterColor(color, status);
    }

    public void setColorWithNoAngle(int color, int status) {
    ...
    public void setColorWheelScale(float startScale) {
        float radius = mColorWheelRadius * startScale;
        mColorWheelRectangle.set(-radius, -radius,
                radius, radius);
        mColorWheelRadiusScale = startScale;
    }
    ...
    private float[] calculatePointerPosition(float angle) {
        float x = (float) (mColorWheelRadius * mColorWheelRadiusScale * Math.cos(angle));
        float y = (float) (mColorWheelRadius * mColorWheelRadiusScale * Math.sin(angle));
     ...
     */
    public void setNewCenterColor(int color, int status) {
        ...
        if (status != S_WITH_NO_RESPONING && onColorChangedListener != null && color != oldChangedListenerColor) {
    ...
    public void setNewCenterColor(int color) {
        setNewCenterColor(color, S_RESPONING);
    }
    ...
    public void changeOpacityBarColor(int color, int status) {
        if (mOpacityBar != null) {
            mOpacityBar.setColor(color, status);
        }
    }
     ...
     * @param color int of the color used to change the opacity bar color.
     */
    public void changeValueBarColor(int color, int status) {
        if (mValueBar != null) {
            mValueBar.setColor(color, status);
        }
    }
    ...
    public void hideColorWheel() {
        mShowColorWheel = false;
        invalidate();
    }

    public void showColorWheel() {
        mShowColorWheel = true;
        invalidate();
    }
}
