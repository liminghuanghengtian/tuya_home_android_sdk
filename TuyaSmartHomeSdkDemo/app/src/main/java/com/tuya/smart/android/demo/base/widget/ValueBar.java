...
//    private final int mDistanceLeft = 200;
//    private final int mDistanceRight = 200;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (!isInEditMode()) {
//            shader = new LinearGradient(mBarPointerHaloRadius - mDistanceLeft, 0,
//                    x1 + mDistanceRight, y1,
...
//                    .round((mBarLength - (mSatToPosFactor * hsvColor[2]) - mDistanceLeft)
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (onValueSelectedListener != null && oldChangedListenerValue != mColor) {
...
//                x1 + mDistanceRight, y1, new int[]{
        ...
        mBarPointerPaint.setColor(mColor);
        if (mPicker != null) {
            mPicker.setNewCenterColor(mColor, status);
            if (mPicker.hasOpacityBar())
                mPicker.changeOpacityBarColor(mColor, status);
        }
        invalidate();
    }

    public void setColor(int color) {
        setColor(color, ColorPicker.S_RESPONING);
    }
     ...
     */
    public void setValue(float value, int status) {
        mBarPointerPosition = Math
    ...
    public int getValue() {
        float v = (mBarLength + mBarPointerHaloRadius - mBarPointerPosition) / mSatToPosFactor;
        if (v > 1) v = 1;
        else if (v < 0) v = 0;
        return (int) (v * 255);
    }
    ...
    public void setValue(float value) {
        setValue(value, ColorPicker.S_RESPONING);
    }

    public void setValue(int value) {
        if (value > 255) value = 255;
        else if (value < 0) value = 0;
        setValue(value / 255.0f);
    }
...
//        coord = coord - mBarPointerHaloRadius - mDistanceLeft;
        coord = coord - mBarPointerHaloRadius;
        if (coord < 0) {
            coord = 0;
//        } else if (coord > mBarLength - mDistanceRight) {
//            coord = mBarLength - mDistanceRight;
